...
type PulseCard = {
  id: string; regionId: string; nameEs: string; nameEn: string;
  tempC: number; tempF: number; humidity: number;
  moodEs: string; moodEn: string; image: string;
};

const PULSE_CARDS: PulseCard[] = [
  { id: 'sjo', regionId: 'sjo', nameEs: 'San José', nameEn: 'San José', tempC: 22, tempF: 72, humidity: 76, moodEs: 'Fresco de ciudad', moodEn: 'Fresh city air', image: REGIONS.find(r => r.id === 'sjo')?.image || '' },
  { id: 'caribe', regionId: 'caribe_sur', nameEs: 'Cahuita', nameEn: 'Cahuita', tempC: 29, tempF: 84, humidity: 82, moodEs: 'Calor caribeño', moodEn: 'Caribbean warmth', image: REGIONS.find(r => r.id === 'caribe_sur')?.image || '' },
  { id: 'carara', regionId: 'manuel_antonio', nameEs: 'Parque Nacional Carara', nameEn: 'Carara National Park', tempC: 30, tempF: 86, humidity: 84, moodEs: 'Sendero tropical', moodEn: 'Tropical trails', image: REGIONS.find(r => r.id === 'manuel_antonio')?.image || '' },
  { id: 'celeste', regionId: 'arenal', nameEs: 'Río Celeste', nameEn: 'Rio Celeste', tempC: 22, tempF: 72, humidity: 90, moodEs: 'Selva y cataratas', moodEn: 'Rainforest & waterfalls', image: REGIONS.find(r => r.id === 'arenal')?.image || '' },
  { id: 'hermosa', regionId: 'guanacaste', nameEs: 'Playa Hermosa', nameEn: 'Playa Hermosa', tempC: 31, tempF: 88, humidity: 68, moodEs: 'Brisa del Pacífico', moodEn: 'Pacific breeze', image: REGIONS.find(r => r.id === 'guanacaste')?.image || '' }
];
    ...
    <section className="relative overflow-hidden bg-[#061b13] py-10 sm:py-14">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 mb-7">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-200"><Sparkles className="w-3.5 h-3.5" />Costa Rica Pulse</div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-3 snap-x snap-mandatory scrollbar-hide">
          {PULSE_CARDS.map(card => {
            ...
            return <motion.button key={card.id} type="button" whileHover={{ y: -3 }} whileTap={{ scale: 0.985 }} onClick={() => setActiveId(card.id)}
              ...
              <div className="absolute inset-x-0 bottom-0 p-4"><div className="flex items-end justify-between gap-3"><div><div className="flex items-center gap-1.5 text-white"><MapPin className="w-3.5 h-3.5 text-emerald-300" /><span className="font-black">{es ? card.nameEs : card.nameEn}</span></div><div className="mt-1 text-[11px] text-white/70">{es ? card.moodEs : card.moodEn}</div></div><div className="text-right"><div className="text-2xl font-black text-white">{Math.round(liveWeather[card.regionId]?.temperatureC ?? card.tempC)}°</div><div className="text-[9px] font-bold text-white/55">{Math.round((liveWeather[card.regionId]?.temperatureC ?? card.tempC) * 9 / 5 + 32)}°F</div></div></div></div>
            </motion.button>;
          })}
        </div>
        ...
        <motion.div key={active.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-4 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-5 rounded-3xl border border-white/10 bg-white/[0.045] p-5 sm:p-6 backdrop-blur-xl">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div><div className="flex items-center gap-2 text-emerald-200/55 text-[10px] uppercase font-black"><Sun className="w-4 h-4" />{es ? 'Temperatura' : 'Temperature'}</div><div className="mt-1 font-bold text-white">{Math.round(currentWeather?.temperatureC ?? active.tempC)}°C / {Math.round((currentWeather?.temperatureC ?? active.tempC) * 9 / 5 + 32)}°F</div></div>
