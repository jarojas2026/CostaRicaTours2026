        ...
        <div className="flex gap-3 overflow-x-auto pb-3 snap-x snap-mandatory scrollbar-hide">
          {PULSE_CARDS.map(card => {
            ...
            return <motion.button key={card.id} type="button" whileHover={{ y: -3 }} whileTap={{ scale: 0.985 }} onClick={() => setActiveId(card.id)}
              ...
              <div className="absolute inset-x-0 bottom-0 p-4"><div className="flex items-end justify-between gap-3"><div><div className="flex items-center gap-1.5 text-white"><MapPin className="w-3.5 h-3.5 text-emerald-300" /><span className="font-black">{es ? card.nameEs : card.nameEn}</span></div><div className="mt-1 text-[11px] text-white/70">{es ? card.moodEs : card.moodEn}</div></div><div className="text-right"><div className="text-2xl font-black text-white">{card.tempC}°</div><div className="text-[9px] font-bold text-white/55">{card.tempF}°F</div></div></div></div>
            </motion.button>;
          })}
        </div>
