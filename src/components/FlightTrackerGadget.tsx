...
export const FlightTrackerGadget: React.FC<FlightTrackerGadgetProps> = ({
  ...
  return (
    <div className="bg-emerald-950/90 border-2 border-emerald-500/30 rounded-[2.5rem] p-5 sm:p-8 shadow-2xl space-y-6 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10 border-b border-emerald-500/20 pb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3.5 py-1 bg-emerald-900/90 text-amber-400 rounded-full text-[11px] font-black uppercase tracking-wider border border-emerald-500/30 mb-2">
        ...
        <div className="flex flex-col items-start md:items-end gap-1">
          ...
          <select
            ...
            className="bg-emerald-900 border-2 border-amber-400/60 rounded-xl px-4 py-2.5 text-xs font-black text-white focus:outline-none focus:border-amber-400 cursor-pointer shadow-md"
          >
            {ORIGIN_COUNTRIES.map((c) => (
              <option key={c.countryCode} value={c.countryCode} className="bg-emerald-950 text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 bg-emerald-900/40 p-4 rounded-2xl border border-emerald-500/20">
        
        {/* Origin Airport within Selected Country */}
        <div className="space-y-1">
          <label className="text-[11px] font-bold text-emerald-200 uppercase flex items-center gap-1">
          ...
          <select
            ...
            className="w-full bg-emerald-950 border border-emerald-500/30 rounded-xl p-2.5 text-xs text-white font-bold focus:outline-none focus:border-amber-400 cursor-pointer"
          >
        ...
        <div className="space-y-1 flex flex-col justify-end">
          <label className="flex items-center gap-2 p-2.5 bg-emerald-950 border border-emerald-500/30 rounded-xl cursor-pointer hover:border-amber-400 transition-colors">
            <input
              type="checkbox"
              checked={directOnly}
        ...
        {/* Max Price Filter */}
        <div className="space-y-1">
          <div className="flex justify-between text-[11px] font-bold text-emerald-200 uppercase">
      ...
      <div className="space-y-4">
        {availableRoutes.length === 0 ? (
          <div className="text-center py-10 bg-emerald-900/30 rounded-2xl border border-emerald-500/20 p-6 space-y-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-emerald-800 hover:bg-emerald-700 text-white rounded-full text-xs font-bold transition-colors cursor-pointer"
            >
          ...
          availableRoutes.map((route) => (
            <div
              key={route.id}
              className="bg-emerald-900/70 hover:bg-emerald-900 border-2 border-emerald-500/30 hover:border-amber-400/80 rounded-2xl p-4 sm:p-5 transition-all shadow-lg hover:shadow-2xl flex flex-col lg:flex-row lg:items-center justify-between gap-5 group"
            >
              {/* Left Column: Airline & Flight Info */}
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-emerald-950 border border-emerald-500/40 flex items-center justify-center font-black text-amber-400 text-sm shadow-inner">
                    {route.airlineCode}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono bg-emerald-950 text-emerald-200 px-2 py-0.5 rounded border border-emerald-600/30">
                        {route.flightNumber}
                      </span>
                    </div>
              ...
              <div className="flex items-center justify-between sm:justify-center gap-4 sm:gap-8 bg-emerald-950/80 p-3 sm:p-4 rounded-xl border border-emerald-500/20 min-w-[280px]">
                {/* Origin */}
              ...
              <div className="flex flex-row lg:flex-col items-center lg:items-end justify-between gap-3 border-t lg:border-t-0 border-emerald-500/20 pt-3 lg:pt-0">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="bg-amber-400 hover:bg-amber-300 text-emerald-950 font-black px-4 py-2.5 rounded-full text-xs uppercase tracking-wider transition-all shadow-md hover:shadow-xl flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
                  >
      ...
      <div className="bg-emerald-900/40 p-4 sm:p-5 rounded-2xl border border-emerald-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="text-[11px] font-bold bg-emerald-950 hover:bg-emerald-800 text-amber-300 px-3 py-1.5 rounded-full border border-emerald-500/30 transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="text-[11px] font-bold bg-emerald-950 hover:bg-emerald-800 text-emerald-200 px-3 py-1.5 rounded-full border border-emerald-500/30 transition-colors cursor-pointer"
          >
