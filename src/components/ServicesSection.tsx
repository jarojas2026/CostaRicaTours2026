...
export const ServicesSection: React.FC<ServicesSectionProps> = ({ language }) => {
  ...
  return (
    <section className="py-16 bg-transparent border-b border-stone-200/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <motion.div
              ...
              className="bg-white/60 backdrop-blur-md p-8 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] hover:shadow-[0_0_30px_rgba(99,102,241,0.2)] border border-black/10 hover:border-orange-500/50 transition-all duration-300 flex flex-col group relative overflow-hidden"
            >
              ...
              <div className="bg-stone-50 w-16 h-16 rounded-2xl flex items-center justify-center mb-6 shadow-md border border-black/10/50 group-hover:scale-110 transition-transform">
                {service.icon}
              </div>
              <h3 className="text-xl font-bold text-white mb-2 uppercase tracking-wide">
                {language === 'es' ? service.titleEs : service.titleEn}
              </h3>
              <p className="text-stone-900/70 leading-relaxed flex-1">
