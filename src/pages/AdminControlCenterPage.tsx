...
export const AdminControlCenterPage: React.FC<Props> = ({ language }) => {
  ...
  return (
    <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 py-7 space-y-6">
      ...
      <section className="mb-6 rounded-3xl border border-cyan-400/20 bg-gradient-to-br from-[#071a22] to-[#0b1018] p-5 shadow-2xl">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-[10px] uppercase tracking-[.22em] text-cyan-300">AI Command Center</div><h2 className="mt-1 text-xl font-black text-white">{es?'Inteligencia empresarial y control humano':'Business intelligence & human control'}</h2><p className="mt-1 text-xs text-stone-400">{es?'La IA analiza y propone; las acciones sensibles requieren aprobación humana.':'AI analyzes and proposes; sensitive actions require human approval.'}</p></div><ShieldCheck className="text-cyan-300" size={30}/></div>
        <div className="mt-4 grid lg:grid-cols-4 gap-3">{[['Ventas netas',kpis.netSalesUSD||0],['Flujo conocido',financial.cashFlow?.netKnownCashFlowUSD||0],['CxC',kpis.accountsReceivableUSD||0],['Margen',kpis.grossMarginUSD||0]].map((item:any)=><div key={String(item[0])} className="rounded-2xl bg-black/25 border border-white/5 p-3"><div className="text-lg font-black text-white">{Number(item[1]).toLocaleString('en-US',{minimumFractionDigits:2})}</div><div className="text-[9px] uppercase text-stone-500">{item[0]}</div></div>)}</div>
        <div className="mt-4 flex flex-col lg:flex-row gap-2"><input value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')runAiAnalysis();}} placeholder={es?'Pídele a la IA analizar ventas, rentabilidad, riesgos, journeys o proponer mejoras...':'Ask AI to analyze sales, profitability, risks, journeys or propose improvements...'} className="flex-1 rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white outline-none focus:border-cyan-300/50"/><button onClick={runAiAnalysis} disabled={aiBusy} className="rounded-xl bg-cyan-400 px-5 py-3 text-sm font-black text-slate-950 disabled:opacity-50">{aiBusy?'Analizando…':(es?'Analizar con IA':'Analyze with AI')}</button></div>
        {aiResult && <pre className="mt-3 max-h-72 overflow-auto rounded-2xl bg-black/40 p-4 text-xs text-cyan-100 whitespace-pre-wrap">{JSON.stringify(aiResult,null,2)}</pre>}
      </section>
