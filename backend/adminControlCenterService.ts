 ...
 * Read-only operational aggregation: bookings, sales, agents, automations,
 * providers, failures, traveler documents and system evolution.
 */
...
export async function getAdminControlCenterSnapshot() {
  const [bookings, alerts, providers, conversion, selfDevelopment] = await Promise.all([
    getAllBookings(),
    getAlerts({ resolved: false }),
    Promise.resolve(getProvidersOverview()),
    getWeeklyConversionMetrics().catch(() => null),
    getSelfDevelopmentOverview().catch(() => null)
  ]);
  ...
  return {
    ...
    business: {
      ...
      failedOrCancelled: failures.length,
      upcoming72h: upcoming.length,
      revenueUSD: revenue,
      averageBookingUSD: confirmed.length ? money(revenue / confirmed.length) : 0
    },
    ...
    alerts: {
      ...
      recent: alerts.slice(0, 15).map((a: any) => ({
    ...
    agents: AGENT_IDENTITIES.map((a: any) => ({
      id: a.id, mission: a.mission, capabilities: a.canRead || [], writableDomains: a.canWrite || [],
      escalation: a.escalation || []
    })),
    automation: {
      engine: getNativeEngineStatus(),
      recentLogs: getNativeAutomationLogs(40),
      providerInbox: {
        mode: 'Gmail OAuth + native cron every minute',
    ...
    evolution: {
      selfDevelopment: selfDevelopment || { enabled: false },
      skills,
      recentEvaluations: recentEvaluations.map(e => ({
        id: e.id, agentId: e.agentId, score: e.score, grounded: e.grounded,
        safety: e.avoidedUnsafeAction, createdAt: e.createdAt, notes: e.notes
      }))
    },
    documents,
