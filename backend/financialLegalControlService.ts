...
const DEFAULTS = {
  ...
  tax_mode: 'inclusive',
  payment_fee_rate: 0,
  provider_commission_rate: 0,
  fiscal_document_required: true,
  electronic_invoicing_enabled: false,
  accounting_period_lock: false,
  require_financial_approval: true,
  require_refund_approval: true,
  require_provider_payout_approval: true,
  legal_review_required_for_policy_changes: true
} as const;
...
async function readControls() {
  ...
  try {
    const snap = await db.collection('financial_controls').doc('global').get();
...
export async function getFinancialLegalControls() {
  return readControls();
}
...
export async function updateFinancialLegalControls(input: Record<string, unknown>) {
  ...
  const numberKeys = new Set(['iva_rate','payment_fee_rate','provider_commission_rate']);
  const allowed = new Set(Object.keys(DEFAULTS));
  const sanitized: Record<string, any> = {};
  for (const [key, value] of Object.entries(input || {})) {
    if (!allowed.has(key)) continue;
    if (booleanKeys.has(key) && typeof value === 'boolean') sanitized[key] = value;
    else if (numberKeys.has(key) && Number.isFinite(Number(value))) sanitized[key] = Math.max(0, Math.min(1, Number(value)));
    else if (key === 'tax_mode' && (value === 'inclusive' || value === 'exclusive')) sanitized[key] = value;
    else if (key === 'currency' && (value === 'USD' || value === 'CRC')) sanitized[key] = value;
  }
  const db = getFirestoreDb();
  if (db && Object.keys(sanitized).length) {
    await db.collection('financial_controls').doc('global').set({ ...sanitized, updatedAt: new Date().toISOString() }, { merge: true });
  }
  return readControls();
}

function isRevenueBooking(b: any) {
  const s = normalizeStatus(b.status);
  const p = normalizeStatus(b.paymentStatus);
  return ['confirmada','confirmed','completada','completed'].includes(s) || ['paid','completed','succeeded'].includes(p);
}

function isCancelled(b: any) {
  return ['cancelada','cancelled','refunded'].includes(normalizeStatus(b.status));
}
...
export async function getFinancialLegalSnapshot() {
  ...
  const providerPayables = money(revenueBookings.reduce((s,b) => {
    ...
    if (num(c.provider_commission_rate) > 0) return s + num(b.totalUSD) * num(c.provider_commission_rate);
    return s;
  }, 0));
  const netSales = money(grossSales - refunds);
  const iva = c.tax_mode === 'inclusive'
    ? money(netSales - (netSales / (1 + num(c.iva_rate))))
    : money(netSales * num(c.iva_rate));
  ...
  const operatingProfitProxy = grossMargin;
  const unpaid = bookings.filter(b => ['pending','pendiente','pendiente_pago','hold'].includes(normalizeStatus(b.paymentStatus || b.status)));
  const unissuedFiscalDocuments = c.fiscal_document_required
    ? revenueBookings.filter(b => !b.electronicInvoice && !b.fiscalDocumentNumber).length
    : 0;
  ...
  const cashCollected = money(revenueBookings.reduce((s,b) => s + num(b.totalUSD), 0) - refunds);

  const paymentMethods: Record<string, number> = {};
  for (const b of revenueBookings) {
    const key = String(b.paymentMethod || 'unknown');
    paymentMethods[key] = money((paymentMethods[key] || 0) + num(b.totalUSD));
  }

  const byTour: Record<string, { bookings:number; revenueUSD:number; providerPayableUSD:number }> = {};
  for (const b of revenueBookings) {
    const key = b.tourName || b.tourId || 'Tour';
    byTour[key] ||= { bookings: 0, revenueUSD: 0, providerPayableUSD: 0 };
    byTour[key].bookings += 1;
    byTour[key].revenueUSD = money(byTour[key].revenueUSD + num(b.totalUSD));
  ...
  return {
    ...
    kpis: {
      ...
      providerPayablesUSD: providerPayables,
      grossMarginUSD: grossMargin,
      operatingProfitProxyUSD: operatingProfitProxy,
      accountsReceivableUSD: accountsReceivable,
      cashCollectedUSD: cashCollected,
      cancelledBookings: cancelled.length,
      unpaidBookings: unpaid.length,
      unissuedFiscalDocuments,
      providerPayoutsPending: payoutPending
    },
    cashFlow: {
      inflowsUSD: cashCollected,
      outflowsKnownUSD: money(refunds + paymentFees + providerPayables),
      netKnownCashFlowUSD: money(cashCollected - refunds - paymentFees - providerPayables)
    },
    paymentMethods,
    tourProfitability: Object.entries(byTour).map(([tour, v]) => ({
      tour, ...v, contributionUSD: money(v.revenueUSD - v.providerPayableUSD)
    })).sort((a,b) => b.contributionUSD - a.contributionUSD).slice(0, 20),
    accounting: {
      basis: 'booking-ledger',
      revenueRecognition: 'confirmed_or_paid_booking',
      openReceivables: accountsReceivable,
      taxMode: c.tax_mode,
      warning: 'Completar gastos operativos, activos, pasivos, cuentas bancarias y asientos contables para estados financieros completos.'
    },
    legal
  };
}
...
async function getLegalComplianceSnapshot(bookings: any[], controls: any) {
  ...
  if (db) {
    try {
      const snap = await db.collection('legal_compliance').orderBy('updatedAt', 'desc').limit(50).get();
  ...
  const merged = required.map(item => {
    const saved = records.find(r => r.id === item.id);
    return { ...item, ...(saved || {}) };
  });
  return {
    complianceScore: Math.round((merged.filter(x => x.status === 'compliant' || x.status === 'configured').length / merged.length) * 100),
    items: merged,
    fiscalDocumentsPending: controls.fiscal_document_required
      ? bookings.filter(isRevenueBooking).filter(b => !b.electronicInvoice && !b.fiscalDocumentNumber).length
      : 0,
    disclaimer: 'El estado es de control interno. La validación jurídica, tributaria y contable debe realizarse con profesionales y fuentes oficiales de Costa Rica.'
  };
}

export async function recordFinancialEvent(event: Record<string, any>) {
  const db = getFirestoreDb();
  const payload = { ...event, createdAt: new Date().toISOString() };
  if (!db) return { stored: false, payload };
  const ref = await db.collection('financial_audit_log').add(payload);
  return { stored: true, id: ref.id, payload };
}
...
export async function getFinancialAuditLog() {
  ...
  if (!db) return [];
  try {
    const snap = await db.collection('financial_audit_log').orderBy('createdAt','desc').limit(100).get();
